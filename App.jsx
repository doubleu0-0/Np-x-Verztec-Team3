...
function AppContent() {
  ...
  const loadChats = async () => {
    try {
      ...
      const response = await fetch('http://172.20.10.2:8000/search?q=', {
        headers: {
  ...
  const handleLogin = async (email) => {
    const token = localStorage.getItem('token');
    const profileRes = await fetch('http://172.20.10.2:8000/profile', {
  ...
  const handleLogout = async () => {
    ...
    if (token) {
      try {
        await fetch('http://172.20.10.2:8000/logout', {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
          },

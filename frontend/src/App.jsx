// Mobile Optimized App.jsx with avatar in left panel and chat search
...
function AppContent() {
  ...
  const [markdownContent, setMarkdownContent] = useState(''); // Added missing state
  ...
  const handleChatSelect = (logId) => {
    ...
    setView('chat');
    setIsSidebarOpen(false);
    setIsSearchOpen(false); // Close search popup when chat is selected
  };
  ...
  return (
    <div className={`flex h-screen overflow-hidden ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-white'}`}>
      ...
      {/* Enhanced Sidebar with Avatar and Chat Search */}
      <div className={`
        fixed lg:relative inset-y-0 left-0 z-50 w-80 lg:w-64 xl:w-80
      ...
      <div className="flex flex-col flex-1 min-w-0">
        ...
        {/* Model Selector and View Buttons */}
      ...
      {/* Search Popup - Moved outside main content area */}
      <SearchPopup
        isOpen={isSearchOpen}
